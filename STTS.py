...
def STTS_sampler(table, i, mu = 0, sd = 1):
    if table[2,i] == 0:
        ...
        mu = table[3,i] + table[3,:].max()/np.sqrt(table[2,i])
...
if __name__ == '__main__':
    ...
    for i in range(cond1_length*cond2_length):
        ...
        bandit_control_table[4,i] = 0.0 # SD
    ...
    print(np.argmax(bandit_control_list[0][4,:]), np.sum(bandit_control_list[0][2,:]))
